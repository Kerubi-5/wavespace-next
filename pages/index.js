...
export default function Home() {
  ...
  const RenderCard = ({ children, item }) => {
    return (
      <>
        <div className="flex items-center gap-2 divide-solid divide-x-2">
          <span>{children}</span>
        ...
        {allWaves.reverse().map((item) => {
          return (
            ...
            >
              ...
              <RenderCard
                item={item.time.toLocaleString("en-GB", { timeZone: "UTC" })}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6"
